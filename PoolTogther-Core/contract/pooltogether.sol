...
contract poolTogether is IPoolTogether, BaseTrust, Ownable, ERC20, ReentrancyGuard{
    ...
    function deposit(uint256 _amountA, uint256 _amountB) external virtual override nonReentrant {
        ...
        require(pool[currentId].state == Status.open, "Can't deposit now");
        ...
        uint256 ticketNumber_ = increasedLP.mul(pool[currendId].poolStartTime + interval - block.timestamp).div(100);
    ...
    function withdraw(uint256 amount) external virtual override nonReentrant {
        ...
        require(pool[currentId].state = Status.open, "Not in proper timimg");
    ...
    function openPoolTogether() external virtual override onlyOwner {
        ...
        pool[currentId].state = Status.open;
    ...
    function drawWinner(uint256 randomNumber) external virtual override onlyOwner {
        ...
        IERC20(aca).burn(earned.mul(500).div(10000)); //Burn 5% of total reward
        earned = earned.mul(9500).div(10000);
        IERC20(aca).transfer(userOrders[pool[currentId].winnerOrder]._user, earned);
        pool[currentId].state = Status.end;
        currentId += 1;
        isDraw = true;
    }
